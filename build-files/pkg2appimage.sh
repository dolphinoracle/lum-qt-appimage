...
if [ ! -z "${_ingredients_dist}" ] ; then
  ...
  echo "deb http://ppa.launchpad.net/djcj/gnutls-patched/ubuntu trusty main" >> sources.list
fi
...
if [ -d usr/lib/x86_64-linux-gnu/glib-2.0/glib-compile-schemas/ ] ; then
  ( cd usr/lib/x86_64-linux-gnu/glib-2.0/glib-compile-schemas ; glib-compile-schemas . )
fi
